...
import { useIsInAppBrowser } from '@/lib/utils/isWebView'
...
export default function RegisterPage() {
  ...
  const [capState, setCapState] = useState<{ allowed: boolean; cap: number; count: number | null } | null>(null)
  const [capLoading, setCapLoading] = useState(true)
  const isWebView = useIsInAppBrowser()
  const router = useRouter()
  const supabase = createClient()

  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText('https://hirecanvas.in')
      toast.success('Link copied — paste it into Chrome or Safari')
    } catch {
      toast.error('Copy failed — open hirecanvas.in in Chrome or Safari')
    }
  }
  ...
  useEffect(() => {
    ...
    const loadCap = async () => {
      try {
        const response = await fetch('/api/beta/cap')
        const data = (await response.json()) as { allowed: boolean; cap: number; count: number | null }
        if (!mounted) return
        setCapState(data)
      } catch {
        if (!mounted) return
        setCapState({ allowed: true, cap: 100, count: null })
      } finally {
        if (mounted) setCapLoading(false)
      }
    }
  ...
  const isAtCapacity = Boolean(capState && !capState.allowed)

  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault()
    if (isAtCapacity) {
      setError('Beta is at capacity. Join the waitlist to get early access.')
      return
    }
    setLoading(true)
    setError(null)
        ...
        <CardContent>
          {isAtCapacity && (
            <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
              We&apos;re at beta capacity. Join the waitlist and we&apos;ll invite you next.{' '}
              <a className="font-semibold underline underline-offset-2" href="mailto:hello@hirecanvas.in?subject=HireCanvas%20Waitlist">
                Join the waitlist
              </a>
              .
            </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <Button type="submit" className="w-full" isLoading={loading} disabled={isAtCapacity || capLoading}>
            ...
            {isWebView ? (
              <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 space-y-2">
                <p className="font-medium">Open in Chrome or Safari to sign up with Google.</p>
                <p className="text-amber-700">
                  Google blocks sign-in inside in-app browsers (WhatsApp, Instagram, LinkedIn).
                </p>
                <button
                  type="button"
                  onClick={() => void handleCopyLink()}
                  className="inline-flex items-center gap-1.5 rounded-lg bg-white border border-amber-300 px-3 py-1.5 font-medium text-amber-800 hover:bg-amber-100"
                >
              ...
              <Button
                ...
                disabled={googleLoading || loading || isAtCapacity || capLoading}
              >
