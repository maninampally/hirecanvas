...
import { SalaryAnalytics } from '@/components/dashboard/SalaryAnalytics'
import { DashboardSkeleton } from '@/components/dashboard/DashboardSkeletons'
import { Badge } from '@/components/ui/badge'
import { DateInput } from '@/components/ui/date-input'
import { useAuthStore } from '@/stores/authStore'
import { useSyncStatus } from '@/hooks/useSyncStatus'
import { getStatusVariant } from '@/lib/statusBadge'
import {
  MdWork,
  MdHandshake,
  MdStarRate,
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const phase = syncStatus?.status
    const prev = prevSyncStatusRef.current
    prevSyncStatusRef.current = phase
    ...
    void loadDashboardData().catch((error) => {
      const message = error instanceof Error ? error.message : 'Failed to refresh after sync'
      setAnalyticsError(message)
      setSyncReportError(message)
    })
  }, [syncStatus?.status, loadDashboardData])

  useEffect(() => {
    const prev = prevExtractingRef.current
    prevExtractingRef.current = extractionInProgress

    if (!prev || extractionInProgress) return

    void loadDashboardData().catch((error) => {
      const message = error instanceof Error ? error.message : 'Failed to refresh after extraction'
      setAnalyticsError(message)
      setSyncReportError(message)
    })
  }, [extractionInProgress, loadDashboardData])
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-6 animate-slide-up delay-250">
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <ResponseTimeHistogram data={analytics?.responseDistribution || []} />
        </div>

        <SalaryAnalytics data={analytics?.salaryAnalytics || []} />
              ...
              {recentJobs.map((job) => {
                return (
                  <tr key={job.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4"><Badge variant={getStatusVariant(job.status)}>{job.status}</Badge></td>
